...
def main(sides: int,
    ...
    doublet = np.empty(3 * n, dtype=np.double)
    source = np.empty(3 * n, dtype=np.double)

    lib = ct.CDLL('./src/pypm3D/bin/velocity/velocity.so')

    lib.panel_velocity.argtypes = [
        ct.c_int,                         # n
        ct.c_int,                         # sides
        Vec2D,                            # p1
        Vec2D,                            # p2
    ...
    lib.panel_velocity.restype = None
    ...
    source = source.reshape((n, 3))
    doublet = doublet.reshape((n, 3))

    return [source, doublet]

def quad_vortex_panel(p1: np.ndarray,
                      p2: np.ndarray,
                      p3: np.ndarray,
                      p4: np.ndarray,
                      p_ctrl: np.ndarray) -> tp.List[np.ndarray]:
    ...
    p1_vec = Vec3D(p1[0], p1[1], p1[2])
    p2_vec = Vec3D(p2[0], p2[1], p2[2])
    p3_vec = Vec3D(p3[0], p3[1], p3[2])
    p4_vec = Vec3D(p4[0], p4[1], p4[2])

    vel = np.empty(3 * n, dtype=np.double)

    lib = ct.CDLL('./src/pypm3D/bin/velocity/velocity.so')

    lib.non_planar_quad_vortice.argtypes = [
        ct.c_int,                         # n
        Vec3D,                            # p1
        Vec3D,                            # p2
        Vec3D,                            # p3
    ...
    lib.non_planar_quad_vortice.restype = None

    lib.non_planar_quad_vortice(
        n,
        p1_vec,
        p2_vec,
        p3_vec,
