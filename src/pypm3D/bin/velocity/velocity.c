...
void vel_vortex_line(Vec3D p, Vec3D p1, Vec3D p2, Vec3D *vel)
{
    Vec3D r0, r1, r2, r1_x_r2;
    double r1_norm, r2_norm, r1_x_r2_norm;

    r0.x = p2.x - p1.x; r0.y = p2.y - p1.y; r0.z = p2.z - p1.z;
    r1.x = p.x - p1.x; r1.y = p.y - p1.y; r1.z = p.z - p1.z;
    r2.x = p.x - p2.x; r2.y = p.y - p2.y; r2.z = p.z - p2.z;
    r1_x_r2 = cross(r1, r2);

    r1_norm = norm(r1);
    r2_norm = norm(r2);
    r1_x_r2_norm = norm(r1_x_r2);

    if ((r1_norm < 1e-8) || (r2_norm < 1e-8) || (r1_x_r2_norm < 1e-8)) {
        vel->x = 0.0;
        vel->y = 0.0;
        vel->z = 0.0;
    } else {
        double k = (1 / (4.0 * M_PI)) * (dot(r0, r1) / r1_norm - dot(r0, r2) / r2_norm) / (r1_x_r2_norm * r1_x_r2_norm);
        vel->x = k * r1_x_r2.x;
        vel->y = k * r1_x_r2.y;
        vel->z = k * r1_x_r2.z;
    }
...
void non_planar_quad_vortice(int n,
                             ...
                             double vel[])
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        vel_vortex_line(p, p1, p2, &v1);
        vel_vortex_line(p, p2, p3, &v2);
        vel_vortex_line(p, p3, p4, &v3);
        vel_vortex_line(p, p4, p1, &v4);

        vel[3 * i] = v1.x + v2.x + v3.x + v4.x;
        vel[3 * i + 1] = v1.y + v2.y + v3.y + v4.y;
        vel[3 * i + 2] = v1.z + v2.z + v3.z + v4.z;
    }
