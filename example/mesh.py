...
def rectangular_wing(th: float, span: float, n_span: int, n_chord: int, show: bool = False):
    """
    ...
    # Points
    p1 = gm.model.geo.add_point(1.0 - 0.5, .0, naca_point(th, 1.0))
    p2 = gm.model.geo.add_point(0.5 - 0.5, .0, naca_point(th, 0.5))
    p3 = gm.model.geo.add_point(0.0 - 0.5, .0, naca_point(th, 0.0))
    p4 = gm.model.geo.add_point(0.5 - 0.5, .0,-naca_point(th, 0.5))
    
    p1e = gm.model.geo.add_point(1.0 - 0.5,-0.5 * span, naca_point(th, 1.0))
    p2e = gm.model.geo.add_point(0.5 - 0.5,-0.5 * span, naca_point(th, 0.5))
    p3e = gm.model.geo.add_point(0.0 - 0.5,-0.5 * span, naca_point(th, 0.0))
    p4e = gm.model.geo.add_point(0.5 - 0.5,-0.5 * span,-naca_point(th, 0.5))

    p1d = gm.model.geo.add_point(1.0 - 0.5, 0.5 * span, naca_point(th, 1.0))
    p2d = gm.model.geo.add_point(0.5 - 0.5, 0.5 * span, naca_point(th, 0.5))
    p3d = gm.model.geo.add_point(0.0 - 0.5, 0.5 * span, naca_point(th, 0.0))
    p4d = gm.model.geo.add_point(0.5 - 0.5, 0.5 * span,-naca_point(th, 0.5))
    ...
    # Curves
    c1 = gm.model.geo.add_polyline([p1] + [gm.model.geo.add_point(x[0], .0, x[1]) for x in foil_1] + [p2])
    c2 = gm.model.geo.add_polyline([p2] + [gm.model.geo.add_point(x[0], .0, x[1]) for x in foil_2] + [p3])
    c3 = gm.model.geo.add_polyline([p3] + [gm.model.geo.add_point(x[0], .0, x[1]) for x in foil_3] + [p4])
    c4 = gm.model.geo.add_polyline([p4] + [gm.model.geo.add_point(x[0], .0, x[1]) for x in foil_4] + [p1])

    c1e = gm.model.geo.add_polyline([p1e] + [gm.model.geo.add_point(x[0],-0.5 * span, x[1]) for x in foil_1] + [p2e])
    c2e = gm.model.geo.add_polyline([p2e] + [gm.model.geo.add_point(x[0],-0.5 * span, x[1]) for x in foil_2] + [p3e])
    c3e = gm.model.geo.add_polyline([p3e] + [gm.model.geo.add_point(x[0],-0.5 * span, x[1]) for x in foil_3] + [p4e])
    c4e = gm.model.geo.add_polyline([p4e] + [gm.model.geo.add_point(x[0],-0.5 * span, x[1]) for x in foil_4] + [p1e])

    c1d = gm.model.geo.add_polyline([p1d] + [gm.model.geo.add_point(x[0], 0.5 * span, x[1]) for x in foil_1] + [p2d])
    c2d = gm.model.geo.add_polyline([p2d] + [gm.model.geo.add_point(x[0], 0.5 * span, x[1]) for x in foil_2] + [p3d])
    c3d = gm.model.geo.add_polyline([p3d] + [gm.model.geo.add_point(x[0], 0.5 * span, x[1]) for x in foil_3] + [p4d])
    c4d = gm.model.geo.add_polyline([p4d] + [gm.model.geo.add_point(x[0], 0.5 * span, x[1]) for x in foil_4] + [p1d])
    ...
    cl5e = gm.model.geo.add_curve_loop([c1e, c2e, c3e, c4e])
    ...
    cl5d = gm.model.geo.add_curve_loop([c1d, c2d, c3d, c4d])
    ...
    # s5e = gm.model.geo.add_surface_filling([cl5e])
    ...
    # s5d = gm.model.geo.add_surface_filling([cl5d])
...
def find_vertices_at_xz_plane(vertices: np.ndarray, y0: float) -> np.ndarray:

    ids = []

    for i in range(vertices.shape[0]):
        if vertices[i, 1] < 1e-3 and vertices[i, 1] > -1e-3:
            ids.append(i)
